...
def get_node_zones(zones_type, node):
    for line in open(BUDDY_INFO):
...
def check_mallocinfo(node, zones_type, node_key):
    empty_zone_tally = 0
    high_order_seq = 0
    zone_info = {"zones": {}}

    node_zones = get_node_zones(zones_type, node)
...
def check_nodes_memory(zones_type):
    node_zones = {}
    ...
    for node in nodes:
        ...
        check_mallocinfo(node, zones_type, node_key)

        if node_key in KERNEL_INFO["memory-checks"]:
            KERNEL_INFO["memory-checks"][node_key].append(node_zones[node_key])
