...
checks:
  isTrue:
    requires:
      and:
        - property: tests.unit.test_ycheck_base.TestProperty.always_true
        - property:
            path: tests.unit.test_ycheck_base.TestProperty.always_false
            ops: [[not_]]
  isalsoTrue:
    requires:
      or:
        - property: tests.unit.test_ycheck_base.TestProperty.always_true
        - property: tests.unit.test_ycheck_base.TestProperty.always_false
  isstillTrue:
    requires:
      and:
        - property: tests.unit.test_ycheck_base.TestProperty.always_true
...
scenarioB:
  checks:
    property_w_error:
      property: tests.unit.test_ycheck_base.TestProperty.i_dont_exist
  conclusions:
    c1:
...
checks:
  cfg_is_bad:
    config:
      handler: tests.unit.test_ycheck_base.TestConfig
      path: test.conf
      assertions:
        - key: key1
          section: DEFAULT
...
vars:
  ...
  fromprop: '@tests.unit.test_ycheck_base.TestProperty.myattr'
...
vars:
  ...
  v1: '@tests.unit.test_ycheck_base.TestProperty.always_false'
  # this one will raise an ImportError
  v2: '@tests.unit.test_ycheck_base.TestProperty.doesntexist'
checks:
