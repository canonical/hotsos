...
class JujuMachine():
    ...
    @cached_property
    def juju_db_snap_channel(self):
        """
        Get the juju-db-snap-channel from agent.conf if it exists.

        This is only present on controller machines.
        """
        return self.config.get("juju-db-snap-channel")
