...
class SequenceSearchDef(object):
    ...
    def __init__(self, start, end, tag, body=None):
        """
        Information required to define and perfom searches for sequences of
        data whereby a sequence must have a matches start and end with optional
        body in between.
        ...
        @param end: SearchDef object for matching end
...
class FileSearcher(object):
    ...
    def _search_task(self, term_key, fd, path):
        ...
        if sequence_results:
            # If we reached end of file and end expr does not match empty
            # string discard the results
            filter_section_idx = []
            for s_term in self.paths[term_key]:
                if type(s_term) == SequenceSearchDef:
                    if s_term.started:
